...
#-------------------------------------------------------------------------
    ...
    observeEvent(input$Target_dummief, {
  
    values$df_data <- fastDummies::dummy_cols(values$df_data, select_columns = input$logistic_response_var, remove_first_dummy = TRUE)
       values$df_data = values$df_data[, -which(names(values$df_data) == input$logistic_response_var)]
      colnames(values$df_data)[colnames(values$df_data) == names(values$df_data)[length(names(values$df_data))]] <- input$logistic_response_var
    ...
    observeEvent(input$train_decision_trees, {
    ...
    # Create the Random Forest model
     observeEvent(input$train_random_forests, {
      ...
      rfModel <-randomForest(formula, data = train_data, ntree = input$rf_ntree)
